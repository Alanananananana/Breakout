...
void GameManager::update(float dt)
{
    ...
    //get mouse coordinates and make sure we're in the window
    const sf::Vector2i mpix = sf::Mouse::getPosition(*_window);
    const sf::Vector2f m = _window->mapPixelToCoords(mpix);

    //get current paddle position
    sf::Vector2f paddlePos = _paddle->GetPosition();

    const auto winSize = _window->getSize();
    const float paddleHalfW = _paddle->getSize() * 0.5f;
    float minX = paddleHalfW;
    float maxX = static_cast<float>(winSize.x) - paddleHalfW;

    paddlePos.x = std::max(minX, std::min(m.x, maxX)); // snap paddle to mouse

    _paddle->SetPosition(paddlePos);
